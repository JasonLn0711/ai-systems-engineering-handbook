...
| LO2 | 把 Day 1 gateway lifecycle 對應到 Day 2 的 route、schema、identity、policy、tool、review 與 audit contract | Gateway alignment note | Gateway mock continuity |
...
7. 是否可以寫入 memory？
8. audit event 要記錄哪些 evidence？
9. 哪些 governance assumptions 要交給 Day 3 red-team？
```
...
| Artifact | 主要評分問題 |
|---|---|
| Gateway alignment note | 是否明確接回 `POST /gateway/requests`、schema validation、trusted identity、policy decision、tool broker、human review、audit event 與 HTTP outcomes？ |
| Governance layer map | 是否包含 identity、registry、tool/data/memory boundary、policy、audit、review、evaluation、red-team？ |
...
| Risk-control map | 是否能把風險連到 control 與 evidence？ |
...
- Gateway alignment note 能清楚連接 `POST /gateway/requests`、schema validation、trusted identity、policy decision、tool broker、review state、audit event 與 HTTP outcomes。
