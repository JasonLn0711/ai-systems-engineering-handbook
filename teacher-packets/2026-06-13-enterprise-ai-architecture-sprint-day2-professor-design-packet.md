...
保留 audit evidence，並能銜接 Day 1 的 minimal AI Gateway mock contract，
才能進入企業系統。
...
1. 學生版教程。
2. 教師授課指南。
3. 課前閱讀與 Day 1 gateway handoff 銜接。
4. 課堂 worksheet。
5. Day 2 gateway alignment 與 governance artifacts 的模板。
...
Day 1 建立 AI Gateway architecture，並把下一步明確交給 minimal gateway mock：
`POST /gateway/requests`、schema validation、trusted identity、action extraction、
policy decision、agent selection、RAG/tool boundary、guardrail、human review
與 audit event。Day 2 把 Gateway 需要管理的 agent、tool、memory、policy、
audit、evaluation 與 review contract 定義出來，並讓這些 governance artifacts
能直接驅動 Day 1 handoff 所描述的 mock gateway handler。Day 3 會把 Day 2 的
治理假設轉成 red-team test cases。
...
- 我能把 Day 1 的 `POST /gateway/requests` lifecycle 對應到 Day 2 的 registry、
  policy、tool broker、memory rule、review state、audit schema 與 HTTP outcome。
...
2. **Gateway alignment contract**：學生能說明 Day 1 的 `POST /gateway/requests`
   如何使用 Day 2 的 agent registry、policy gate、tool broker、memory rule、
   review state、audit schema 與 HTTP outcome。
3. **Contract or schema**：學生能寫出 agent registration、policy gate、
   audit event 的欄位。
4. **Mechanism explanation**：學生能說明為什麼 prompt 不等於權限控制。
5. **Failure-mode reasoning**：學生能把缺少 governance 的狀態對應到具體風險。
6. **Review evidence**：TA 能從學生 artifact 判斷設計是否合格。
7. **Next-day handoff**：Day 2 產出的 policy assumption 可以直接變成 Day 3
   red-team test cases。
...
-> Day 1-to-Day 2 gateway alignment
...
與 red-team 測試的 governance contract。合格的 Day 2 artifact 必須能回答：
minimal gateway mock 收到一個 request 時，會依據哪些 registry、policy、tool、
memory、review、audit 欄位決定 `allow`、`deny`、`review_required` 或 HTTP
failure outcome。
...
1. 學生能說明為什麼 enterprise agent 需要 registration、ownership、task scope、
   ...
   與 review states。
2. 學生能把 Day 1 gateway lifecycle 對應到 Day 2 的 route、schema、policy、
   tool、review 與 audit contract。
3. 學生能區分 common governance rules 與 adapter-specific behavior。
4. 學生能為一個 public-safe enterprise agent 撰寫 agent registration record。
5. 學生能撰寫 policy gate，並讓 policy decision 明確回傳 `allow`、`deny` 或
   `review`。
6. 學生能設計 audit event schema，記錄 identity、agent、tool、source、memory、
   policy、guardrail、review 與 outcome。
7. 學生能把 tool abuse、memory leakage、permission bypass、prompt-only approval、
   missing audit detail 對應到具體 governance controls。
8. 學生能產出 Day 3 red-team seeds，讓治理假設變成可測試案例。

每個目標都要對應到學生 artifact 與 rubric category：

| 學習目標 | 學生 evidence | 評分項目 |
|---|---|---|
| Governance purpose | worksheet 短答 | governance layer map |
| Day 1-to-Day 2 continuity | gateway alignment note | gateway mock continuity |
| Common vs adapter | common-vs-adapter table | common-vs-adapter separation |
...
### 10.2 Gateway Alignment Note 是 implementation bridge

Day 2 必須新增一個明確的 `Day 1-to-Day 2 gateway alignment and message mediation note`。這份 note
不是程式碼，也不是抽象心得，而是把 Day 1 的 minimal gateway mock contract
轉成 Day 2 governance 設計的對照表。
...
Campus IT Helpdesk Agent 的示例可以是：

```text
Route: POST /gateway/requests
Request schema fields: session_token, channel, raw_message, client_hints, requested_agent
Trusted identity source: verified demo session token resolved server-side
Policy decisions needed: allow public FAQ, deny staff-only SOP, review ticket submission
Read-only tool: search_it_faq
Side-effect tool: submit_ticket
Message mediation path: gateway handler or support-safe broker topic; raw staff-only payload must not enter a student-readable channel
Declassification or redaction rule: staff-only SOP content is not returned to student role; ticket draft removes unnecessary private context
Replay or expiration rule: request expires after one session; repeated side-effect action uses idempotency key
Expected HTTP outcomes: 200 completed or pending_review, 400 malformed input, 401 invalid token, 403 denied staff-only access
```

軟體工程重點：

- route 與 handler 是 governance 被執行的地方。
- `client_hints` 是提示，不是可信任權限來源。
- identity、role、permission、policy 與 data access 必須 server-side resolve。
- HTTP status 與 response body 要讓 `allow`、`deny`、`review_required` 可觀察。
- audit event 要證明 gateway handler 實際使用了 policy gate 與 tool broker。
- 高權限 agent 不能成為低權限 agent 的代讀器。老師可以用 OS 中 privileged
  helper process 的類比教學生：低權限使用者即使不能直接讀 root-only file，
  也可能透過高權限 process 的 output、log、cache、pipe、socket、shared memory
  或暫存檔間接拿到內容。
- Kafka-like broker 可以降低耦合並形成中介點，但它不是完整安全邊界；完整設計
  還需要 identity、authorization policy、data classification、declassification、
  audit、rate limit、schema governance、replay protection、tool sandbox、
  network isolation 與 human review。

### 10.3 Tool Boundary 是 side-effect control
...
### 10.4 Policy Gate 是 decision API
...
### 10.5 Audit Event 是 lifecycle evidence
...
- 第一結論。
- Day 1 到 Day 2 的 gateway contract 銜接。
...
6. governance table 沒有連回 `POST /gateway/requests` route、HTTP outcome 或 audit event。
...
- expected HTTP status 或 gateway outcome。
...
### 14.1 Day 1-To-Day 2 Gateway Alignment Note

學生要把 Day 1 handoff 的 minimal gateway mock contract 對應到 Day 2 governance
artifact：
...
- 明確寫出 `POST /gateway/requests` 或等價 route。
- 說明 request schema 如何承接 `session_token`、`raw_message`、`client_hints`
  與 `requested_agent`。
- 說明可信任 identity、role、permission 由 server-side 解析，而不是信任 client。
- 至少包含一個 `allow`、一個 `deny`、一個 `review_required` 路徑。
- 至少包含 `200`、`400`、`401`、`403` 四種可觀察 outcome。
- audit fields 能證明 policy、retrieval、tool、memory、guardrail、review decision。
- 說明跨信任邊界的 agent、tool、retrieval service 或 process 互動要走 mediation
  path，而不是直接互相呼叫。
- 說明 broker topic、queue、gateway 或 tool gateway 的 producer、consumer、
  classification、retention、replay protection 與 raw payload policy。

### 14.2 Governance Layer Map
...
- 至少包含 identity、agent、tool、data、memory、message mediation、policy、
  audit、review。
- 每層有一句責任說明。
- 能說明哪一層負責 enforcement，哪一層負責 evidence。

### 14.3 Common-Vs-Adapter Table
...
| Message mediation | schema、topic/queue ACL、producer/consumer identity、retention、replay control | support-safe request/result channel；student agent 不可讀 raw staff-only topic |
...
### 14.4 Agent Registration Record
...
| Confused deputy | 低權限 agent 請高權限 agent 讀 staff SOP 並回傳摘要 | original user、requesting agent、resource、purpose、output classification 一起做 policy check | denied delegation decision |
| Broker payload leakage | staff-only 原文被 publish 到所有 agent 可讀的 shared topic | topic ACL、no raw secret payload、consumer allowlist、short retention | broker audit、schema validation |
| Replay of privileged result | 舊的高權限結果在授權過期後被重新 consume | expires_at、nonce、idempotency key、decision_id、consume 前重新檢查 policy | replay denial audit |
| Missing schema field | message 沒有 classification 卻被當 public 處理 | required schema fields、schema registry、default deny | schema rejection event |
...
10. 如果 `POST /gateway/requests` 回傳 `review_required`，audit event 必須記錄哪些欄位才能證明 review gate 被執行？
11. 如果低權限 agent 請高權限 agent 代讀資料，這是合法 delegation 還是 confused deputy？
12. 如果使用 Kafka-like broker，哪些資料可以放在 message 裡，哪些只能放 resource reference 或 redacted result？
...
| governance artifacts 是表格就好 | artifacts 必須能驅動 gateway route、HTTP outcome、review state 與 audit evidence |
| 加上 Kafka 就安全 | broker 只控制訊息通道，仍需要 identity、policy、classification、declassification、audit、replay protection 與 network isolation |
| 高權限 agent 可以幫低權限 agent 摘要任何資料 | 高權限 agent 仍要依原始使用者、目的、resource、output classification 做 policy check |
...
- Day 1-to-Day 2 gateway alignment and message mediation note 能清楚連接 `POST /gateway/requests`、
  schema validation、trusted identity、policy decision、tool broker、review state、
  message mediation、declassification、audit event 與 HTTP outcomes。
- policy gate、tool boundary、memory scope、message mediation、audit event 都有具體
  schema 或 template。
...
不是只靠 prompt 管理，而是要透過 Day 1-to-Day 2 gateway alignment、
message mediation、agent registry、tool boundary、data boundary、memory scope、
policy gate、declassification、audit event、human review、evaluation hook 與
...
prompt injection、tool abuse、memory leakage、permission bypass、HTTP outcome
mismatch、confused deputy、broker payload leakage、replay 與 audit evasion test
cases。
