...
4. 高權限 agent、tool、process 不能成為低權限使用者或 agent 的代讀器。
...
8. 如果低權限 agent 請高權限 agent 摘要 staff-only 文件，這是合法 delegation 還是 confused deputy？
9. Kafka-like broker 能解決哪些耦合問題？哪些安全問題仍然必須靠 policy、schema、audit、declassification 解決？
...
| 「高權限 agent 可以幫低權限 agent 看資料」 | 忽略 original user / purpose / output classification | 用 confused deputy 和 OS privileged helper 類比修正 |
| 「加 Kafka 就安全」 | 把 broker 當成完整安全邊界 | 補 identity、topic ACL、schema、declassification、replay、DLQ controls |
...
-> message mediation / declassification check
-> policy decision
-> review state
-> audit event
-> HTTP outcome
...
- 跨 agent、process、tool 的請求要走 gateway、broker、queue 或 tool gateway。
- broker 只能控制通道，不能取代 authorization、declassification、audit 或 network isolation。
...
| Message Mediation | schema、topic ACL、producer/consumer identity、retention、replay control | support-safe topics、raw privileged topic isolation |
...
- message_channel 與 output_classification 要說明高敏感資料是否被降敏後才跨 zone。
...
4. Message mediation 是否防止 high-privilege agent 代讀？
...
5. Governance layers：identity、registry、tool、data、memory、message mediation、policy、review、audit、eval、red-team。
5a. Mediation layer：gateway / broker / tool gateway, no raw privileged payloads to low-trust channels。
...
### Failure 7：High-privilege agent 成為 low-privilege agent 的代讀器

學生 artifact：

```text
student_agent cannot read staff_only_sop.
student_agent asks staff_agent to summarize staff_only_sop.
staff_agent returns the summary to student_agent.
```

問題：資料庫 ACL 可能仍然正確，但輸出路徑洩密。policy 不能只看 `staff_agent`
有沒有讀取權限，也要看 original user、requesting agent、purpose、resource、
...
### Failure 8：Broker 被當成 shared secret pipe

學生 artifact：

```text
topic: agent.messages
producer: staff_agent
consumer: all_agents
payload: raw staff-only SOP text
```

問題：Kafka-like broker 降低耦合，但不會自動做資料降敏、original-user 授權、
schema governance、DLQ 保護或 output classification。raw secret payload 進入 broker
後，可能被 replica、consumer buffer、debug trace、dead-letter queue、monitoring
system 保存。
...
[ ] message mediation 有 producer、consumer、classification、schema、retention、replay、DLQ 規則。
...
[ ] PII 最小化。
[ ] audit 記錄 message_channel 與 output_classification。
...
|---|---|
| Gateway alignment and message mediation note | 產生 HTTP outcome、message channel、declassification tests |
...
6. confused deputy seed。
7. broker payload leakage seed。
8. replay or missing schema field seed。
...
- Message mediation 防止 high-privilege agent 成為 low-privilege agent 的代讀器。
