...
This means the product can still begin with a simple prompt box. The constraint
is not on the user; the constraint is on execution.
...
The first design is natural-language-first for the user and policy-first inside
the system. The second design feels convenient at first, but it can execute the
wrong tool, read the wrong data, or create an irreversible side effect.
...
The hardest practical pain point is that real users do not speak in JSON
schemas. A student or staff member may write:

```text
幫我處理一下 VPN，那個帳號好像又壞了，順便開單給 IT。
```

This one prompt is not one clean action. It may contain:
...
So the classifier is not only answering "what category is this?" A useful
gateway classifier produces a structured proposal:
...
The important detail is multi-label classification. The prompt is not:
...
That is why a single-label classifier is usually too weak for enterprise AI
Gateway design.
...
|---|---|---|
| UI controlled fields | stable internal workflows, category chips, urgency selector | less flexible if forced before the user explains the problem |
| rule-based parser | high-risk verbs such as delete, send, reset, export, open permission | brittle for varied language; best used as risk signal |
| traditional classifier | repeated categories such as IT, billing, HR, VPN, account lock | needs labeled examples; weak for multi-intent prompts |
| transformer classifier | semantic variants and multi-label probabilities | needs training/evaluation data and threshold tuning |
| embedding retrieval over action registry | finding known tools before the planner chooses | requires a maintained action/tool registry |
| LLM structured output | flexible action extraction and slot filling | must be schema-validated and policy-checked |
...
For the complex VPN prompt, a richer proposal may look like this:
...
UI hints are useful when they help the gateway interpret the request without
forcing the user to complete a long form. A poor UX makes the user serve the
schema:
...
A better UX lets the user start naturally, then shows a small interpretation
preview:
...
The user did not have to learn the schema. The gateway uses the schema behind
the scenes. If fields are missing, ask only the minimum necessary question:
...
Direct prompt input lowers typing cost. Gateway previews, confidence
thresholds, and confirmation lower correction cost and side-effect risk.

Use this safe-default table:
...
For the VPN example, a good first response is:
