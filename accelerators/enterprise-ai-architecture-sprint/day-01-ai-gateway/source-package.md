...
| 方法 | 適合情境 | 弱點 | Day 1 建議 |
|---|---|---|---|
| UI controlled fields | 固定流程，例如問題類型、緊急程度、希望動作 | 使用者彈性較低 | 一定要有 |
| Rule-based parser | 「建立 ticket」「寄信」「查 SOP」這類明確 trigger | 語言變化大時 brittle | 先做最小可行版本 |
| Traditional classifier | 大量重複分類，例如 IT、billing、HR、complaint | 需要 labeled data | 作為中期改進 |
